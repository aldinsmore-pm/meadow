  ...
  return (
    ...
    >
      <div className="text-center md:text-left">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ember/70">Selective partnerships</p>
      ...
      <ul className="grid gap-3 md:grid-cols-2">
        {trustNotes.map((note, index) => (
          <li
            key={note}
            className="flex items-start gap-3 rounded-xl border border-ember/20 bg-linen/80 p-4 text-sm text-ink shadow-soft"
            data-reveal
            data-reveal-delay={180 + index * 40}
          >
            <span className="mt-0.5 inline-flex h-6 w-6 flex-none items-center justify-center rounded-full border border-ember/30 bg-ember/15 font-serif text-sm font-semibold text-ember">
              ✓
            </span>
